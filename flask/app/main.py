...
def add_bool_column_if_not_exists(db_path, table, column, default):
  conn = sqlite3.connect(db_path)
  cursor = conn.cursor()
  cursor.execute(f"PRAGMA table_info({table})")
  columns = [info[1] for info in cursor.fetchall()]
  if column not in columns:
    cursor.execute(f"ALTER TABLE {table} ADD COLUMN {column} BOOLEAN NOT NULL DEFAULT {default}")
    conn.commit()
  conn.close()
...
with app.app_context():
  ...
  # Add 'published' column to jwellery table if missing
  add_bool_column_if_not_exists(DB_PATH, 'jwellery', 'published', 0)
...
def admin_dashboard():
  ...
  total_images = Jwellery.query.count()
  zero_quantity_images = Jwellery.query.filter_by(quantity=0).count()
  return render_template('admin_dashboard.html', total_images=total_images, zero_quantity_images=zero_quantity_images)
...
def admin_jwellery():
  if not session.get('admin_logged_in'):
    return redirect(url_for('admin_login'))
  if request.method == 'POST':
    # Handle price, quantity, and published update for existing image
    update_id = request.form.get('update_id')
    update_price = request.form.get('update_price')
    update_quantity = request.form.get('update_quantity')
    update_published = request.form.get('update_published')
    if update_id and (update_price is not None or update_quantity is not None or update_published is not None):
      try:
    ...
    for img in images:
      if img and img.filename:
        ...
        img.save(save_path)
        db.session.add(Jwellery(image_path=filename))
    if images:
      db.session.commit()
